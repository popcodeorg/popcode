...
test('openAssignmentCreator', (assert) => {
  ...
  testSaga(openAssignmentCreatorSaga).
    next().call(getCourses, pageToken).
    next({courses, nextPageToken}).put(coursesLoaded(courses)).
    next().put(coursesFullyLoaded()).
    next().isDone();
  assert.end();
});
...
test('createAssignmentSaga', (t) => {
  ...
  const url = 'http://localhost:3001/?snapshot=54a8a2a0-d2f2-427e-95d6-f58ec371673f';
  ...
  t.test('plublished', (assert) => {
    ...
    testSaga(createAssignmentSaga, {payload: {
      ...
      next(assignment).put(assignmentCreated({
  ...
  t.test('draft', (assert) => {
    ...
    testSaga(createAssignmentSaga, {payload: {
      ...
      next(assignment).select(getCourse, selectedCourseId).
      next(course()).put(assignmentCreated({
